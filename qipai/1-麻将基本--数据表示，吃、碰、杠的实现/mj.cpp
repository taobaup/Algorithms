...
{
	int op = 0;
	if (card_index == 0 || card_index > 28) 
		return false;

	if (card_index == 1 || card_index == 10 || card_index == 19)
	{
		if (cards[card_index + 1] > 0 && cards[card_index + 2] > 0) 
			op = OP_left_chi;
	...
	if (cards[card_index + 1] > 0 && cards[card_index - 1] > 0) 
		op |= OP_center_chi;

	return op;
}

void chi(char* cards, std::vector<CHI_PENG_GANG>& chi_peng_gang, 
			char card_index, OP_TYPE op)
{
	CHI_PENG_GANG cpg;
					...
					char card_index, int left_cards_num)
{
	...
	for (std::vector<CHI_PENG_GANG>::iterator it = chi_peng_gang.begin(); 
			it != chi_peng_gang.end(); ++it)
	{
